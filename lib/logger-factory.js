...
class LoggerFactory {
  ...
  static validateOptions(opt) {
    let allLevels = ['fatal', 'error', 'warn', 'info', 'debug', 'trace'];

    let notSupportedList = _.difference(opt.logLevels, allLevels);
  ...
  createStandandStreams() {
    ...
    let stdoutLevels = ['info', 'debug', 'trace'];
    ...
    for (let level of this.opt.logLevels) {
      streamList.push({
        level: level,
        type: 'stream',
        stream: stderrLevels.indexOf(level) > -1 ? process.stderr :
          (stdoutLevels.indexOf(level) > -1 ? process.stdout : undefined)
      });
    }
