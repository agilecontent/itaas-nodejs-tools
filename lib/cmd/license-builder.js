...
`);
  ...
  static getDepsDetails() {
    return LicenseBuilder.readDepsDetails()
      .then(LicenseBuilder.parseDepsDetails);
  }

  static create3tdPartyLicense(depsDetails, opt) {
    let header = opt.header;
    let allowedLicenseList = opt.allowedLicenseList;
    let file = opt.file;
    let skipPrefix = opt.skipPrefix;

    return Promise.resolve(depsDetails)
      .then((depsDetails) => {
        LicenseBuilder.prepareLicenseFile(file);
        LicenseBuilder.writeLicenseHeader(file, header);

        for (let depencenceName in depsDetails) {
          if (depencenceName.startsWith(skipPrefix)) {
            continue;
          }

          let licenseInfo = depsDetails[depencenceName];
